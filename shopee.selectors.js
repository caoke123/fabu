...
export const SELECTORS = {
  ...
  entry: {
    ...
    // 稳定性：中
    applyBtnCard:
      '#app-main-container > div > div > div.content > div.scs-invitation-items > div > div.invitation-respond-action > button',
    applyBtnCardFallback:
      '.invitation-respond-action button:has-text("立即报名")',

    // 「立即报名」—— 抽屉内确认按钮
    // 稳定性：中
    applyBtnDrawer:
      '#app-main-container > div > div > div.ssc-drawer-container > div.ssc-drawer-wrap > div > div > div.ssc-drawer-body > div > div:nth-child(1) > div.info-section-wrapper > div.info-action > div > span > span > span.ssc-dropdown-reference > button',
    applyBtnDrawerFallback:
      '.ssc-drawer-body button:has-text("立即报名")',

    // 「直接提名」下拉选项
    // 稳定性：低（body > span 位置不固定）⚠️ 只用 Fallback
    directNominateOption:
      'body > span > div > div > div:nth-child(2)',
    directNominateOptionFallback:
      '.ssc-dropdown-menu div:has-text("直接提名")',

    // 进入发布页后的确认弹窗按钮（步骤 8.1）
    // 稳定性：中
    confirmDialogBtn:
      'body > div.ssc-message-box-wrapper > div.ssc-message-box > div > div.ssc-message-box-footer > div.ssc-message-box-footer-primary > button.ssc-button.ssc-message-box-action-button.ssc-btn-type-primary',
    confirmDialogBtnFallback:
      '.ssc-message-box-action-button.ssc-btn-type-primary',
  },
  ...
  basicInfo: {
    ...
    // 描述图片弹窗「本地上传」选项（步骤 17.1）
    // 稳定性：低（body > div:nth-child(46) 位置不固定）⚠️ 优先用 Fallback
    descriptionImageLocalUpload:
      'body > div:nth-child(46) > div > ul > div > div > li',
    descriptionImageLocalUploadFallback:
      '.ssc-dropdown-menu li:has-text("本地上传")',
  ...
  sku: {
    ...
    // 稳定性：中
    addVariantBtn:
      '#app-main-container > div > div > div > section.listing-layout-content-section > div.listing-layout-content-container.listing-layout-content-container-shadow > section.listing-layout-content > div:nth-child(3) > div.scs-variations > section > div > form > div > div > section > div > button',
    ...
    // JIT 是否直发 触发器（步骤 29）
    tableJitTrigger:
      'table > tbody:nth-child(3) > tr:nth-child(1) > td:nth-child(8) > div > div > div > div > span > div > span > span:nth-child(1) > div > div',

    // JIT 选项「是」（步骤 30）
    // 稳定性：低 ⚠️ 只用 Fallback
    tableJitYes:
      'body > span:nth-child(40) > div > div > div > ul > li.ssc-option.ssc-option-highlighted > span',
    tableJitYesFallback:
      '.ssc-option:has-text("是")',
...
export function getSkuRowSelectors(rowIndex) {
  ...
  return {
    ...
    sellingPrice:   cell(7),
    jitTrigger:     `${base} > td:nth-child(8)  span:nth-child(1) > div > div`,
    minOrder:       cell(10),
    skuCode:        cell(11),
    weight:         cell(13),
    length:         cell(14),
